 package
{
	...
	public class Avatar
	{
		...
		//contenant les images chargé (loader) ainsi que les zone texte
		...
		private function loadXMLcompleted(event:Event)
		{
			...
			for each(var Avatar:XML in fileXML..avatar) 
			{
				...
				for each(var Orientation:XML in Avatar..orientation)
				{
					...
					trace("t::"+Orientation.url);
					loadIMG.load(new URLRequest(Orientation.url));					
					tabPhoto[Avatar.@typeAvatar][Avatar.@statureAvatar][Orientation.@degres]["loader"] = loadIMG;
		...
		public static function  getImage(type:String,stature:String,orientation:String):Loader	
		{
			trace("je lis le tableau xml"); 
			return tabPhoto[type][stature][orientation]["loader"] as Loader;
