package
{
	...
	public class Vue3D extends Sprite
	{
		...
		{												
			...
			for(var i:* in ListeActeur)
			{
				if (true)//(ListeActeur[i].isEnVue(pers)) //si la personne est ds le champ de vision
				{
		...
		public function TourneDroite():void
		{
			...
			for(var i:* in ListeActeur)
			{
				var angleTemp:Number = (Acteur)(ListeActeur[i]).getAngleVue()+2;
				if(angleTemp > 0){
					angleTemp = angleTemp%360;
				}else if(angleTemp < 0){
					angleTemp = angleTemp%(-360);
				}
				if(!pers.isEnVue((Acteur)(ListeActeur[i]))){
