		...
		********************************/
		...
		public override function affiche3D() : void {
			...
			if (this.nbPieds == 1) {
				...
				var hauteurPieds:Number = rayon*1.2;				
				spritePieds.graphics.lineStyle(2, this.couleurPieds, 100);						
				spritePieds.graphics.moveTo(coinAvG_x3D,coinAvG_y3D);	      	
		      	spritePieds.graphics.lineTo(coinAvG_x3D,coinAvG_y3D+hauteurPieds);	      	      	
		      	spritePieds.graphics.moveTo(coinAvD_x3D,coinAvD_y3D);
		      	spritePieds.graphics.lineTo(coinAvD_x3D,coinAvD_y3D+hauteurPieds);	     	      	
		      	spritePieds.graphics.moveTo(coinArrG_x3D,coinArrG_y3D);
		      	spritePieds.graphics.lineTo(coinArrG_x3D,coinArrG_y3D+hauteurPieds);  	  	
		      	spritePieds.graphics.moveTo(coinArrD_x3D,coinArrD_y3D);
		      	spritePieds.graphics.lineTo(coinArrD_x3D,coinArrD_y3D+hauteurPieds); 
		...
		/***********************************
		 * calcul pour savoir si un acteur est dans l'angle de vue
		 * (renvoie true) ou pas (renvoie false)
		 ***********************************/
		public override function isEnVue(persProprio:Personne):Boolean
		{
			trace("anglePOX : " + angleCentre);
			if(angleCentre > 20 && angleCentre < 160){
				trace(" oui ");
				return true;
			}
			if(angleCentre > -340 && angleCentre < -180){
				trace(" oui ");
				return true;
			}
			else{
		 ...
		 *********************************************************/
		public override function setAnglePOX(pers:Personne,nbDegre:Number):void {
			//centre
			angleCentre +=nbDegre;
			//sert à garder l'angle dans l'intervalle [0°,360°] lors des incrémentat°
			if (angleCentre >= 360){angleCentre = 360 - angleCentre;}
			else if (angleCentre <= 0) {angleCentre = 360 + angleCentre;}	
						
			angleCoinAvG +=nbDegre;
			//sert à garder l'angle dans l'intervalle [0°,360°] lors des incrémentat°
			if (angleCoinAvG >= 360){angleCoinAvG = 360 - angleCoinAvG;}
			else if (angleCoinAvG <= 0) {angleCoinAvG = 360 + angleCoinAvG;}
			
			angleCoinAvD +=nbDegre;
			//sert à garder l'angle dans l'intervalle [0°,360°] lors des incrémentat°
			if (angleCoinAvD >= 360){angleCoinAvD = 360 - angleCoinAvD;}
			else if (angleCoinAvD <= 0) {angleCoinAvD = 360 + angleCoinAvD;}
			
			angleCoinArrG +=nbDegre;
			//sert à garder l'angle dans l'intervalle [0°,360°] lors des incrémentat°
			if (angleCoinArrG >= 360){angleCoinArrG = 360 - angleCoinArrG;}
			else if (angleCoinArrG <= 0) {angleCoinArrG = 360 + angleCoinArrG;}
			
			angleCoinArrD +=nbDegre;
			//sert à garder l'angle dans l'intervalle [0°,360°] lors des incrémentat°
			if (angleCoinArrD >= 360){angleCoinArrD = 360 - angleCoinArrD;}
			else if (angleCoinArrD <= 0) {angleCoinArrD = 360 + angleCoinArrD;}
			//trace("tourner tab");
		}
