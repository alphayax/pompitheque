package 
{
	...
	public class Table extends Acteur {
		...
		//Constructeur
		public function Table(nom:String, x2D:Number, y2D:Number, angleAbsolu:Number) {
			super(nom, x2D, y2D, angleAbsolu);
			var dimension:Number = 150; //on defini la largeur
...
/*			trace("centreA :"+getX3D()+" , "+getY3D());
			...
			if (this.nbPieds == 1) {
				...
				spritePieds.graphics.moveTo(this.getX3D()-this.x,this.getY3D()-this.y);	      	
		      	spritePieds.graphics.lineTo(this.getX3D()-this.x,this.getY3D()+rayon-this.y);	      	      	
			}
			else{
				
				spritePieds.graphics.lineStyle(2, this.couleurPieds, 100);						
				spritePieds.graphics.moveTo(coinAvG_x3D-this.x,coinAvG_y3D-this.y);	      	
		      	spritePieds.graphics.lineTo(coinAvG_x3D-this.x,coinAvG_y3D+rayon-this.y);	      	      	
		      	spritePieds.graphics.moveTo(coinAvD_x3D-this.x,coinAvD_y3D-this.y);
		      	spritePieds.graphics.lineTo(coinAvD_x3D-this.x,coinAvD_y3D+rayon-this.y);	     	      	
		      	spritePieds.graphics.moveTo(coinArrG_x3D-this.x,coinArrG_y3D-this.y);
		      	spritePieds.graphics.lineTo(coinArrG_x3D-this.x,coinArrG_y3D+rayon-this.y);  	  	
		      	spritePieds.graphics.moveTo(coinArrD_x3D-this.x,coinArrD_y3D-this.y);
		      	spritePieds.graphics.lineTo(coinArrD_x3D-this.x,coinArrD_y3D+rayon-this.y); 
	     	...
	     	shapeSurface3D.graphics.beginFill(this.couleurPlateau, 100);      		    		      		 	      		      		      	
	      	shapeSurface3D.graphics.moveTo(coinAvG_x3D-this.x,coinAvG_y3D-this.y);	      	
	      	shapeSurface3D.graphics.lineTo(coinAvD_x3D-this.x,coinAvD_y3D-this.y);	      	      	
	      	shapeSurface3D.graphics.lineTo(coinArrD_x3D-this.x,coinArrD_y3D-this.y);	      	      	
	      	shapeSurface3D.graphics.lineTo(coinArrG_x3D-this.x,coinArrG_y3D-this.y);  	  	
	      	shapeSurface3D.graphics.lineTo(coinAvG_x3D-this.x,coinAvG_y3D-this.y);					        	 	 			     		
